...
def print_debts_for_all_matrices(project_data, name_of_project):
    ...
    for owner, matrix in participants.items():
        ...
        for col_idx in range(matrix.shape[1]):
            column_sum = np.sum(matrix[:, col_idx])  # Sum of transactions in each column
            participant_name = list(participants.keys())[col_idx]  # Get the participant name based on index
            print(f"{participant_name} owes {owner} the sum of: {column_sum}")
        print("\n")
